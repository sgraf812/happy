...
happyDoAction i tk st =
  ...
  case happyDecodeAction (happyNextAction i st) of
    ...
    HappyAccept -> DEBUG_TRACE("accept.\n")
                   happyAccept i tk st
    HappyReduce rule -> DEBUG_TRACE("reduce (rule " ++ show rule ++ ")")
...
--     = happyFixupFailed tk st sts stk
...
-- happySpecReduce_3 :: FAST_INT -> _ -> FAST_INT -> _
...
-- parse error if we are in fixup and try to reduce
...
-- happyTryFixup :: FAST_INT -> Token
--   -> HappyState Token (HappyStk HappyAbsSyn -> P a)
--   -> [HappyState Token (HappyStk HappyAbsSyn -> P a)]
--   -> _
-- happyFail :: FAST_INT -> FAST_INT -> Token
--   -> HappyState Token (HappyStk HappyAbsSyn -> P a)
--   -> [HappyState Token (HappyStk HappyAbsSyn -> P a)]
--   -> _
...
happyResume i tk st sts stk = pop_items st sts stk
  where
    pop_items st sts stk
      ...
      = DEBUG_TRACE("discarding state " ++ show IBOX(st) ++ "\n")
...
--    discard_input_until_exp :: Happy_GHC_Exts.Int# -> Token -> Happy_GHC_Exts.Int#  -> _
    discard_input_until_exp i tk st sts stk
      ...
      = DEBUG_TRACE("discard token in state " ++ show st ++ ": " ++ show i ++ "\n")
        -- happyLex (happyFmap1 (\a -> a `happySeq` Just a) (happyNewToken st sts stk))                         -- eof
        happyLex (case happyEofTok of
          IBOX(eof) | EQ(i,eof) -> happyReturn1 Nothing
                    | HappyFail <- happyDecodeAction (happyNextAction eof st) -> happyReturn1 Nothing
                    | otherwise -> happyFmap1 (\a -> a `happySeq` Just a) (happyNewToken st sts stk))                             -- eof
--        happyLex (happyReturn1 Nothing)                             -- eof
                 (\i tk -> discard_input_until_exp i tk st sts stk) -- not eof
      | otherwise
      = DEBUG_TRACE("found expected token in state " ++ show st ++ ": " ++ show i ++ "\n")
